...
async function main() {
  ...
  try {
    ...
    if (raw.length === 0) throw new Error('No items fetched — aborting.');
    ...
    for (const [clusterName, items] of Object.entries(clusters)) {
      for (const item of items) item.cluster = clusterName;
    }
    ...
    const toolLaunchesOut = [...summarizedTools,   ...toolLaunches.slice(3)].slice(0, 5);
    const fundingOut      = [...summarizedFunding, ...funding.slice(3)].slice(0, 5);
